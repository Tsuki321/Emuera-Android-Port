...
internal sealed class Utils
{
	...
	static public SKBitmap LoadImage(string filepath)
	{
		...
		try { return SKBitmap.Decode(filepath); } catch { return null; }
	}
		// ビットマップファイルからアイコンファイルをつくる（Windows専用 — 非対応プラットフォームでは未実装）
	public static object MakeIconFromBmpFile(SKBitmap bmp)
	{
		throw new NotImplementedException("MakeIconFromBmpFile requires Windows GDI.");
	}
