...
internal static class Config
{
	...
	public static void SetConfig(ConfigData instance)
	{
		...
		if (FontSize < 8)
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.TooSmallFontSize.Text, trmb.ConfigError.Text).GetAwaiter().GetResult();
			FontSize = 8;
		}
		if (LineHeight < FontSize)
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.LineHeightLessThanFontSize.Text, trmb.ConfigError.Text).GetAwaiter().GetResult();
			LineHeight = FontSize;
		}
		if (SaveDataNos < 20)
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.TooSmallDisplaySaveData.Text, trmb.ConfigError.Text).GetAwaiter().GetResult();
			SaveDataNos = 20;
		}
		if (SaveDataNos > 80)
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.TooLargeDisplaySaveData.Text, trmb.ConfigError.Text).GetAwaiter().GetResult();
			SaveDataNos = 80;
		}
		if (MaxLog < 500)
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.TooSmallLogSize.Text, trmb.ConfigError.Text).GetAwaiter().GetResult();
			MaxLog = 500;
		}
	...
	static readonly Dictionary<string, Dictionary<EngineFontStyle, EngineFont>> fontDic = new Dictionary<string, Dictionary<EngineFontStyle, EngineFont>>();
	public static EngineFont Font { get { return GetFont(null, EngineFontStyle.Regular); } }
	...
	public static EngineFont GetFont(string theFontname, EngineFontStyle style)
	{
		...
		if (!fontDic.TryGetValue(fn, out var fontStyleDic))
		{
			...
			fontDic[fn] = fontStyleDic;
		}
		if (!fontStyleDic.TryGetValue(style, out var font))
		{
			font = new EngineFont(fn, FontSize, style);
			fontStyleDic[style] = font;
		}
		return font;
	}
	...
	private static void createSavDirAndMoveFiles()
	{
		...
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.FailedCreateSavFolder.Text, trmb.FolderCreationFailure.Text).GetAwaiter().GetResult();
			return;
		}
		#region eee_カレントディレクトリー
		...
		if (!existGlobal && savFiles.Length == 0)
			return;
		bool result = GlobalStatic.Dialogs?.ShowYesNoAsync(trmb.SavFolderCreated.Text, trmb.DataTransfer.Text).GetAwaiter().GetResult() ?? false;
		if (!result)
			return;
		//ダイアログが開いている間にフォルダを消してしまうような邪悪なユーザーがいるかもしれない
		if (!Directory.Exists(SavDir))
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.MissingSavFolder.Text, trmb.DataTransferFailure.Text).GetAwaiter().GetResult();
			return;
		}
		...
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.FailedMoveSavFiles.Text, trmb.DataTransferFailure.Text).GetAwaiter().GetResult();
		}
	}
	//先にSetConfigを呼ぶこと
	//戻り値はセーブが必要かどうか
	...
	private static long getUpdateKey()
	{
		...
		string[] erbFiles = PathHelper.GetFilesIgnoreCase(Program.ErbDir, "*.ERB", option);
		string[] csvFiles = PathHelper.GetFilesIgnoreCase(Program.CsvDir, "*.CSV", option);
	...
	private static List<KeyValuePair<string, string>> getFiles(string dir, string rootdir, string pattern, bool toponly, bool sort)
	{
		...
		//filepathsは完全パスである
		string[] filepaths = PathHelper.GetFilesIgnoreCase(dir, pattern, SearchOption.TopDirectoryOnly);
		if (sort)
