...
using System.IO;
using System.Text;
// System.Windows.Forms removed — not available on Android.
using MinorShift.Emuera.GameProc.Function;
using MinorShift.Emuera.Platform;
using FontStyle = MinorShift.Emuera.Platform.EngineFontStyle;
using Font = MinorShift.Emuera.Platform.EngineFont;
...
internal sealed partial class EmueraConsole : IDisposable
{
	...
	public void SetBgColor(Color color)
	{
		...
		//最初の再描画時に現在の背景色に合わせる
		if (redraw == ConsoleRedraw.None && window?.ScrollPosition == window?.MaxScrollPosition)
			return;
	...
	ScanBreak:
		...
		if (errorStr != null)
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.IllegalFontError.Text, trmb.IllegalFontError.Text);
			this.Quit();
			return;
		}
		if (force_LEFT)
		...
		line.LineNo = lineNo;
		//Bitmap Cache
		// bitmapCacheEnabled removed — bitmap cache not used in cross-platform rendering
	...
	private string CreateTypeCString(string str, bool alignmentRight)
	{
		...
		Font font = Config.GetFont(Style.Fontname, Style.FontStyle);
	...
	private bool outputLog(string fullpath)
	{
		...
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.FailedOutputLogError.Text, trmb.FailedOutputLog.Text);
			return false;
		}
		finally
		{
	...
	public bool OutputLog(string filename)
	{
		...
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.CanNotOutputToParentDirectory.Text, trmb.FailedOutputLog.Text);
			return false;
		}
		if (!filename.StartsWith(Program.WorkingDir, StringComparison.CurrentCultureIgnoreCase))
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trmb.CanOnlyOutputToSubDirectory.Text, trmb.FailedOutputLog.Text);
			return false;
		}
