...
internal sealed partial class EmueraConsole : IDisposable
{
	...
	//Bitmap cache removed — rendering moved to platform host.

	public EmueraConsole(IConsoleHost hostArg)
	{
		window = hostArg;
		#region EE_AnchorのCB機能移植
		CBProc = new ClipboardProcessor();
		#endregion
		...
		timer.Elapsed += (s, e) => tickTimer(s, e);
		CBG_Clear();//文字列描画用ダミー追加

		redrawTimer = new System.Timers.Timer(10);
		redrawTimer.AutoReset = false;//TODO:1824アニメ用再描画タイマー有効化関数の追加
		redrawTimer.Elapsed += (s, e) => tickRedrawTimer(s, e);
	}
	#region 1823 cbg関連
...
#pragma warning restore CS0414
	...
	public int ClientWidth { get { return window?.ConsoleWidth ?? Config.WindowX; } }
	public int ClientHeight { get { return window?.ConsoleHeight ?? (Config.LineHeight * 30); } }
	#endregion

	const string ErrorButtonsText = "__openFileWithDebug__";
	private readonly IConsoleHost window;
	#region EE_MOUSEB
	public IConsoleHost Window { get { return window; } }
	#endregion
	#region EE_BINPUT
	...
	// bakedBackground (Bitmap) removed — background baking is now delegated to the platform rendering host.
	...
	#endregion
	public bool Enabled { get { return window?.IsAvailable ?? false; } }

	/// <summary>
	/// 現在、Emueraがアクティブかどうか
	/// </summary>
	internal bool IsActive
	{ get { return window?.IsAvailable ?? false; } }
	...
	public void Initialize()
	{
		...
		GlobalStatic.ConsoleHost = window;
		...
		{
			OpenDebugDialog();
			// window.Focus() - no-op in cross-platform host
		}
		ClearDisplay();
	...
	public void ForceQuit()
	{
		...
		{
			bool result = GlobalStatic.Dialogs?.ShowYesNoAsync(trmb.ForceQuitAndRestart.Text, "FORCE_QUIT_AND_RESTART").GetAwaiter().GetResult() ?? false;
			if (result)
			{
		...
		GlobalStatic.Lifecycle?.RequestExit();
		return;
	}
	#endregion
	...
	{
		// System.Media.SystemSounds.Hand.Play() removed — Windows-only audio feedback.
...
#pragma warning restore CS0414
	...
	public void WaitInput(InputRequest req)
	{
		...
		if (req.Timelimit > 0)
		{
			// update_lastinput() removed — no platform equivalent needed
			presetTimer();
			//				setTimer();
		}
		//updateMousePosition();
	...
	public void AddBackgroundImage(string name, long depth, float opacity)
	{
		...
		// BakeBackground() removed — background rendering delegated to platform host
	}
	...
	// ValidateBackground and BakeBackground removed — GDI+ Bitmap-based baking not available on Android.
	// The platform rendering host receives the backgroundList directly and renders it on its own canvas.
	/// <summary>
	/// INPUT中のアニメーション用タイマー
	/// </summary>
	System.Timers.Timer redrawTimer = null;
	...
	private void tickRedrawTimer(object sender, EventArgs e)
	{
		...
		window?.RequestRedraw();//RequestRedraw発行
	}
...
#pragma warning restore CS0414
	...
	private void endTimer()
	{
		...
		if (IsWaitingPrimitive)
		{
			...
			if (state == ConsoleState.WaitInput && inputReq.NeedValue)
			{
				System.Drawing.Point? cpPoint1 = window?.GetConsoleRelativeCursorPosition();
				if (cpPoint1.HasValue)
					MoveMouse(cpPoint1.Value);
			}
			RefreshStrings(true);
			#endregion
			return;
		...
		if (state == ConsoleState.WaitInput && inputReq.NeedValue)
		{
			System.Drawing.Point? cpPoint2 = window?.GetConsoleRelativeCursorPosition();
			if (cpPoint2.HasValue)
				MoveMouse(cpPoint2.Value);
		}
		RefreshStrings(true);
	}
	...
	private bool doInputToEmueraProgram(string str)
	{
		...
		// 入力成功した
		if (window?.InputBoxPositionChanged ?? false)
			window?.ResetInputBoxPosition();
		#endregion
		return true;
	}
	#endregion
	...
	internal void MouseDown(Point point, int button)
	{
		if (!IsWaitingPrimitive)
			return;
		//pointはクライアント左上基準の座標。
	...
	internal void InputMouseKey(int type, int result1, int result2, int result3, int result4, long result5)
	{
		...
		try
		{
			...
			if (state == ConsoleState.WaitInput && inputReq.NeedValue)
			{
				System.Drawing.Point? cpPoint3 = window?.GetConsoleRelativeCursorPosition();
				if (cpPoint3.HasValue)
					MoveMouse(cpPoint3.Value);
			}
		}
		finally
		{
	...
	endMacro:
		if (state == ConsoleState.WaitInput && inputReq.NeedValue)
		{
			System.Drawing.Point? cpPoint4 = window?.GetConsoleRelativeCursorPosition();
			if (cpPoint4.HasValue)
				MoveMouse(cpPoint4.Value);
		}
		RefreshStrings(true);
	}
	...
	void doSystemCommand(string command)
	{
		...
		{
			GlobalStatic.Lifecycle?.RequestRestart();
			return;
		}
	...
	public void SetWindowTitle(string str)
	{
		...
		{
			debugTitle = str;
			window?.SetWindowTitle(str + " (Debug Mode)");
		}
		else
	...
	{
		if (window != null) window.InputBoxText = str;
	}
	...
	{
		bool isBackLog = (window?.ScrollPosition ?? 0) != (window?.MaxScrollPosition ?? 0);
		...
		if (forceTextBoxColor)
		{
			...
			window?.SetInputBoxBackColor(this.bgColor);
	...
	{
		int pointY = (window?.ConsoleHeight ?? Config.LineHeight * 30) - Config.LineHeight;
		int bottomLineNo = (window?.ScrollPosition ?? 1) - 1;
	...
	#endregion
#if false // OnPaint + WinForms ToolTip handlers removed — WinForms GDI rendering not available on Android
	/// <summary>
...
#endif // end OnPaint + ToolTip handlers

	public void CustomToolTip(bool b)
	{
		// no-op in cross-platform host
	}
	...
	long tooltip_format = 0; // was TextFormatFlags — unused after ToolTip_Draw removed
	bool tooltip_img = false;
	public void SetToolTipDuration(int duration)
	{
		tooltip_duration = duration;
		// window.ToolTip.AutoPopDelay removed — no-op in cross-platform host
	}
	...
	{
		tooltip_format = f; // was (TextFormatFlags) cast, now stored as long
	}
...
#pragma warning restore CS0414 // フィールド 'EmueraConsole.dTraceLogChanged' が割り当てられていますが、値は使用されていません。
	...
	{
		System.Drawing.Point? pos = window?.GetConsoleRelativeCursorPosition();
		if (!pos.HasValue)
			return new Point();
		//クライアント左下基準の座標に置き換え
		Point adjusted = pos.Value;
		adjusted.Y -= ClientHeight;
		return adjusted;
	}
	#region EE_MOUSEB
	...
	private void verticalScrollBarUpdate()
	{
		...
		int move = max - window.MaxScrollPosition;
		if (move == 0)
			return;
		if (move > 0)
		{
			window.MaxScrollPosition = max;
			window.ScrollPosition += move;
		}
		else
		{
			if (max > window.ScrollPosition)
				window.ScrollPosition = max;
			window.MaxScrollPosition = max;
		}
		// ScrollBar.Enabled is managed by the platform host
	}
	#endregion
	#endregion
	...
	public void ReloadErb()
	{
		if (state == ConsoleState.Error)
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trerror.CanNotUseWhenError.Text, "");
			return;
		}
		if (state == ConsoleState.Initializing)
		{
			GlobalStatic.Dialogs?.ShowInfoAsync(trerror.CanNotUseWhenInitialize.Text, "");
			return;
		}
