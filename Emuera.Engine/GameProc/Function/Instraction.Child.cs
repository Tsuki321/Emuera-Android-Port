...
internal sealed partial class FunctionIdentifier
{
	...
	private sealed class INPUT_Instruction : AbstractInstruction
	{
		...
		public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
		{
			...
			// ApplyTextBoxChanges() is WinForms-specific; no-op on non-Windows
			#endregion
			#region EE_INPUT機能拡張
		...
		public override void DoInstruction(ExpressionMediator exm, InstructionLine func, ProcessState state)
		{
			...
			try
			{
				...
				try
				{
					...
					{
						bool result = GlobalStatic.Dialogs?.ShowYesNoAsync(string.Format(trmb.NewVersionAvailable.Text, version, link), trmb.UpdateCheck.Text).GetAwaiter().GetResult() ?? false;
						if (result)
						{
