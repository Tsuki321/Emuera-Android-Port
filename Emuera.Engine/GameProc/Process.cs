...
internal sealed partial class Process(EmueraConsole view, bool analysisMode)
{
	...
	public bool Initialize()
	{
		...
		try
		{
			...
			if (ParserMediator.HasWarning)
			{
				...
				if (GlobalStatic.Dialogs?.ShowYesNoAsync(trmb.ConfigFileError.Text, trmb.ConfigError.Text).GetAwaiter().GetResult() == true)
				{
			...
			if (Config.UseReplaceFile && !analysisMode)
			{
				...
				if (File.Exists(replacePath))
				{
					...
					{
						ParserMediator.FlushWarningList();
						if (GlobalStatic.Dialogs?.ShowYesNoAsync(trmb.ReplaceFileError.Text, trmb.ReplaceError.Text).GetAwaiter().GetResult() == true)
						{
	...
	private void checkInfiniteLoop()
	{
		...
		bool wantsExit = GlobalStatic.Dialogs?.ShowYesNoAsync(text, caption).GetAwaiter().GetResult() ?? false;
		if (wantsExit)
		{
