...
internal static partial class FunctionMethodCreator
{
	...
	private sealed class MoveTextBoxMethod : FunctionMethod
	{
		...
		public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
		{
			if (resume) GlobalStatic.ConsoleHost?.ResetInputBoxPosition();
			// else: SetTextBoxPos is WinForms-specific; no-op on non-Windows
			return 1;
		}
	}
	#endregion
	...
	private sealed class CheckfontMethod : FunctionMethod
	{
		...
		public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
		{
			...
			{
				Int64 isInstalled = 0;
				// Check SkiaSharp system fonts
				foreach (string familyName in SkiaSharp.SKFontManager.Default.FontFamilies)
				{
					if (familyName == str)
					{
						isInstalled = 1;
				...
				// Check custom loaded fonts
				if (isInstalled == 0)
				{
					foreach (string familyName in GlobalStatic.CustomFontFamilyNames)
					{
						if (familyName == str)
						{
							isInstalled = 1;
		...
		public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
		{
			...
			switch (Name)
			{
				...
				case "GGETFONTSTYLE":
					return g.Fontstyle;
				case "GGETPEN":
					return g.PenColor.ToArgb() & 0xffffffffL;
				case "GGETPENWIDTH":
					return (long)g.PenWidth;
				case "GGETBRUSH":
					return g.BrushColor.ToArgb() & 0xffffffffL;
					#endregion
			}
		...
		public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
		{
			...
			try
			{
				...
				foreach (string familyName in GlobalStatic.CustomFontFamilyNames)
				{
					...
					{
						styledFont = Config.GetFont(fontname, fs);
						goto foundfont;
					}
				}
				styledFont = Config.GetFont(fontname, fs);
	...
	public sealed class GraphicsDrawStringMethod : FunctionMethod
	{
		...
		public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
		{
			...
			// Measure text size using SkiaSharp
			EngineFont efont = g.Fnt ?? Config.GetFont(Config.FontName, GlobalStatic.Console.StringStyle.FontStyle);
			SKFontStyle skStyle = new SKFontStyle(
				efont.IsBold ? SKFontStyleWeight.Bold : SKFontStyleWeight.Normal,
				SKFontStyleWidth.Normal,
				efont.IsItalic ? SKFontStyleSlant.Italic : SKFontStyleSlant.Upright);
			using var typeface = SKTypeface.FromFamilyName(efont.FamilyName, skStyle) ?? SKTypeface.Default;
			using var paint = new SKPaint { Typeface = typeface, TextSize = efont.SizeInPixels };
			var skBounds = new SKRect();
			float textWidth = paint.MeasureText(text, ref skBounds);
			Int64[] resultArray = exm.VEvaluator.RESULT_ARRAY;
			resultArray[1] = (Int64)textWidth;
			resultArray[2] = (Int64)skBounds.Height;
			return 1;
		}
	}
	#endregion
	...
	public sealed class GraphicsGetTextSizeMethod : FunctionMethod
	{
		...
		public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
		{
			...
			// Measure text size using SkiaSharp
			EngineFont fnt = Config.GetFont(fontname, fs);
			SKFontStyle skStyle2 = new SKFontStyle(
				fnt.IsBold ? SKFontStyleWeight.Bold : SKFontStyleWeight.Normal,
				SKFontStyleWidth.Normal,
				fnt.IsItalic ? SKFontStyleSlant.Italic : SKFontStyleSlant.Upright);
			using var typeface2 = SKTypeface.FromFamilyName(fnt.FamilyName, skStyle2) ?? SKTypeface.Default;
			using var paint2 = new SKPaint { Typeface = typeface2, TextSize = fnt.SizeInPixels };
			var skBounds2 = new SKRect();
			float textWidth2 = paint2.MeasureText(text, ref skBounds2);
			Int64[] resultArray = exm.VEvaluator.RESULT_ARRAY;
			resultArray[1] = (Int64)skBounds2.Height;
			return (Int64)textWidth2;
		}
	}
	#endregion
	...
	**/
	...
	private sealed class GetTextBoxMethod : FunctionMethod
	{
		...
		public override string GetStrValue(ExpressionMediator exm, IOperandTerm[] arguments)
		{
			return GlobalStatic.ConsoleHost?.InputBoxText ?? "";
		}
	}
	private sealed class ChangeTextBoxMethod : FunctionMethod
	{
		...
		public override Int64 GetIntValue(ExpressionMediator exm, IOperandTerm[] arguments)
		{
			if (GlobalStatic.ConsoleHost != null)
				GlobalStatic.ConsoleHost.InputBoxText = arguments[0].GetStrValue(exm);
			return 1;
		}
	}
	#endregion
