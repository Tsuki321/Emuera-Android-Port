...
internal sealed class GameBase
{
	...
	{
		basePath = Sub.PathHelper.FindFileCaseInsensitive(basePath);
			...
			while ((st = eReader.ReadEnabledLine()) != null)
			{
				...
				switch (tokens[0])
				{
					...
					case "動作に必要なEmueraのバージョン":
						...
						Version curerntVersion = new(GlobalStatic.InternalEmueraVer);
						Version targetVersoin = new(Compatible_EmueraVer);
						if (curerntVersion < targetVersoin)
						{
							ParserMediator.Warn(string.Format(trerror.RequireLaterEmuera.Text, GlobalStatic.EmueraVerText), pos, 2);
							return false;
						}
						break;
					#region EE_UPDATECHECK
