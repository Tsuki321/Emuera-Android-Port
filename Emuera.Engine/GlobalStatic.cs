using System;
using System.Collections.Generic;
using System.Text;
using MinorShift.Emuera.GameProc;
using MinorShift.Emuera.GameData;
using MinorShift.Emuera.GameData.Expression;
using MinorShift.Emuera.GameData.Variable;
using MinorShift.Emuera.GameView;
using MinorShift.Emuera.Platform;

namespace MinorShift.Emuera;

/* 1756 作成
 * できるだけデータはprivateにして必要なものだけが参照するようにしようという設計だったのは今は昔。
 * 改変のたびにProcess.Instance.XXXなんかがどんどん増えていく。
 * まあ、増えるのは仕方ないと諦める事にして、行儀の悪い参照の仕方をするものたちをせめて一箇所に集めて管理しようという計画である。
 * これからはInstanceを public static に解放することはやめ、ここから参照する。
 * しかし、できるならここからの参照は減らしたい。
 */
internal static class GlobalStatic
{
	//これは生成される順序で並んでいる。
	//下から上を参照した場合、nullを返されることがある。

	// ── Platform services (registered by host before Initialize()) ─────────
	public static IPlatformDialogs  Dialogs   { get; set; }
	public static IPlatformPaths    Paths     { get; set; }
	public static IPlatformSound    Sound     { get; set; }
	public static IPlatformLifecycle Lifecycle { get; set; }
	public static IConsoleHost      ConsoleHost { get; set; }

	/// <summary>Internal version string (e.g. "1.823.6.1") set by the host at startup.</summary>
	public static string InternalEmueraVer { get; set; } = "0.0.0.0";
	/// <summary>Human-readable version text set by the host at startup.</summary>
	public static string EmueraVerText { get; set; } = "0.0.0.0";

	// ── Engine objects ─────────────────────────────────────────────────────
	//Config Replace
	public static EmueraConsole Console;
	public static Process Process;
	//Config.RenameDic
	public static GameBase GameBaseData;
	public static ConstantData ConstantData;
	public static VariableData VariableData;
	//StrForm
	public static VariableEvaluator VEvaluator;
	public static IdentifierDictionary IdentifierDictionary;
	public static ExpressionMediator EMediator;
	//
	public static LabelDictionary LabelDictionary;

	// ── Custom font file paths for SkiaSharp loading ──────────────────────
	/// <summary>Absolute paths to custom TTF/OTF font files loaded at startup.</summary>
	public static readonly List<string> CustomFontPaths = [];
	/// <summary>Family names of custom fonts loaded from CustomFontPaths.</summary>
	public static readonly List<string> CustomFontFamilyNames = [];

	//ERBloaderに引数解析の結果を渡すための橋渡し変数
	//1756 Processから移動。Program.AnalysisMode用
	public static Dictionary<string, Int64> tempDic = [];
	#region EE_FORCE_QUIT_AND_RESTART
	public static bool ForceQuitAndRestart = false;//連続実行を防ぐ
	#endregion

#if DEBUG
	public static List<FunctionLabelLine> StackList = [];
#endif
	public static void Reset()
	{
		Process = null;
		ConstantData = null;
		GameBaseData = null;
		EMediator = null;
		VEvaluator = null;
		VariableData = null;
		Console = null;
		LabelDictionary = null;
		IdentifierDictionary = null;
		tempDic.Clear();
		CustomFontPaths.Clear();
		CustomFontFamilyNames.Clear();
	}
}
